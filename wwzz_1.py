...
################################################################################
loose_muon_ip = (muons.pt>5) & (abs(muons.eta)<2.4) & (abs(muons.dxy)<0.05) & (abs(muons.dz)<0.1)  & (muons.pfRelIso04_all<0.4) & (muons.sip3d < 8) & (muons.isPFcand==1 ) & (muons.looseId==1)
veto_muon_sel = (muons.pt>10) & (abs(muons.eta)<2.4)
loose_muon_sel = loose_muon_ip 
tight_muon_sel = (muons.pt>10) & (abs(muons.eta)<2.4) & (abs(muons.dxy)<0.05) & (abs(muons.dz)<0.1)  & (muons.pfRelIso04_all<0.4) & (muons.sip3d < 8) & (muons.isPFcand==1 ) & (muons.mediumId==1) & (muons.mvaTTH > 0.25)
...
(electrons.sieie < 0.011) & (electrons.hoe < 0.1) & (electrons.eInvMinusPInv > -0.04) & (electrons.convVeto ==1) & (electrons.mvaTTH > -0.6))
...
(electrons.sieie < 0.030) & (electrons.hoe < 0.1) & (electrons.eInvMinusPInv > -0.04) & (electrons.convVeto ==1) & (electrons.mvaTTH > -0.6))
...
################################################################################
# 1. should we consider tau? (when I generate nanoaod, I don't consider tau)
# 2. I'm not sure All except 2-prong(+π0) is : decay mode !=3
# 3. taus.idDecayModeNewDMs isn't exist, can I use taus.idDecayModeOldDMs ==0 
#################################################################################
tight_tau_sel = ((taus.pt > 20) & (abs(taus.eta) < 2.3) & (abs(taus.dz) < 0.2) & (abs(taus.dxy) < 1000) & (taus.idDecayModeOldDMs ==0 ) & (taus.decayMode!=3) &
...
good_jets_sel = (jets_drclean_leptons) & (jets.pt > 20) & (abs(jets.eta)<2.4) & (jets.score<0.0490) # add btag
