...
tight_muon_sel = (muons.pt>10) & (abs(muons.eta)<2.4) & (abs(muons.dxy)<0.05) & (abs(muons.dz)<0.1)  & (muons.pfRelIso04_all<0.4) & (muons.sip3d < 8) & (muons.mediumId==1)  
...
njet_sel = (ak.count(good_jets.pt,axis=1)>=2) 

nlep_sel = (ak.count(good_leptons.pt,axis=1)==4) 

npassed = len(events.run)
print('npassed_before_cuts: %d'%npassed)
total_sel_1 = nlep_sel
total_sel_1 = total_sel_1[:len(events)]
# total_sel = nlep_sel
...
lepton1 = leptons[:,0]
lepton2 = leptons[:,1]
mll = (lepton1 + lepton2).mass 
mll_cut = (mll > 25) & (mll < 100)
...
MET = MET[nlep_sel_3]
npassed = len(events.run)
print('npassed_mllsel: %d'%npassed)
...
lepton_pair_sel = lepton_pair_sel[:len(events)]
...
print('npassed_leptonpair: %d'%npassed)
